...
const StudentDashboard = () => {
  ...
  const { subscribeToRoute, notifications, socketConnected } = useNotifications();
  ...
  const hasLoadedNotifications = React.useRef(false);
  ...
  // Load notifications from new system once when user data is available
  useEffect(() => {
    if (userData?._id && !hasLoadedNotifications.current) {
      fetchNotifications();
      hasLoadedNotifications.current = true;
    }
  }, [userData?._id]);

  // Initialize Socket.IO for real-time bus tracking
  useEffect(() => {
    if (userData && userData.selectedRoute) {
      const newSocket = io('http://localhost:5000');
      
      newSocket.on('connect', () => {
        console.log('[StudentDashboard] socket connected:', newSocket.id);
        newSocket.emit('authenticate', { userId: userData._id });
      });
      
      newSocket.on('socketConnected', () => {
        console.log('[StudentDashboard] socket authenticated, subscribing to route:', userData.selectedRoute);
        newSocket.emit('subscribeToRoute', userData.selectedRoute);
      });
      
      newSocket.on('subscriptionConfirmed', () => {
        console.log('[StudentDashboard] subscriptionConfirmed for route:', userData.selectedRoute);
      });
      ...
      newSocket.on('busLocationUpdate', (data) => {
        ...
        // prepend latest update and cap list size
        setLiveUpdates(prev => [{
      ...
      // Clear all live updates when driver completes the trip
      newSocket.on('driverUpdatesClear', (data) => {
        console.log('[StudentDashboard] driverUpdatesClear received:', data);
        setLiveUpdates([]);
        setBusUpdateMessage('✅ Trip completed. All live updates cleared.');
      });
      
      newSocket.on('socketError', (err) => {
        console.error('[StudentDashboard] socketError:', err);
      });
  ...
  const fetchNotifications = async () => {
    try {
      ...
      // Check if there are new notifications (only if we had previous notifications)
      if (localNotifications.length > 0 && response.data.length > localNotifications.length) {
        const newCount = response.data.length - localNotifications.length;
        setToastMessage(`📢 ${newCount} new notification${newCount === 1 ? '' : 's'} arrived!`);
        setShowToast(true);
        setTimeout(() => setShowToast(false), 5000); // Hide after 5 seconds
      }
      
      setLocalNotifications(response.data); // Store notifications locally
  ...
  // Additional safety check - prevent multiple subscriptions
  useEffect(() => {}, [socketConnected]);

  // Reset subscription flag when user data changes
  useEffect(() => {
    if (userData?._id) {
  ...
  // Debug notifications state - only log when notifications actually change
  useEffect(() => {
    if (notifications && notifications.length > 0) {
      if (notifications.length > localNotifications.length) {
        const newCount = notifications.length - localNotifications.length;
        setToastMessage(`📢 ${newCount} new notification${newCount === 1 ? '' : 's'} arrived!`);
        setShowToast(true);
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 relative">
      ...
      {/* Toast Notification - removed by setting showToast to false */}
      {showToast && (
        <div className="fixed bottom-4 right-4 bg-white rounded-lg shadow-lg border border-gray-200 p-4 max-w-sm flex items-center gap-3 z-50 animate-fade-in-up">
      ...
      <div className="fixed bottom-4 left-4 z-50">
        <div
          className={`bg-white p-3 rounded-full shadow-lg cursor-pointer hover:bg-indigo-50 transition-colors ${localNotifications.length > 0 ? 'ring-2 ring-indigo-500' : ''}`}
        ...
        {localNotifications.length > 0 && (
          <div className="absolute bottom-full left-0 mb-2 bg-white rounded-lg shadow-md border border-gray-200 p-2 px-3 text-sm whitespace-nowrap animate-fade-in">
            <span>You have notifications!</span>
          </div>
        )}
      </div>
    </div>
