...
const NotificationSender = ({ showMessage }) => {
  const { sendNotification, on, off, socketConnected, getSocketStatus } = useNotifications();
  ...
  useEffect(() => {
    const handleNotificationSent = (data) => {
      console.log('Notification sent confirmation:', data);
      showMessage(`✅ Notification delivered!`);
    };

    on('notificationSent', handleNotificationSent);

    return () => {
      off('notificationSent', handleNotificationSent);
    };
  }, [showMessage, on, off]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Real-time broadcast is now handled by the backend controller (REST API)
      // so we don't need to emit via socket here anymore.
      ...
      setLastSentNotification({
        ...
        targetType: formData.targetType,
        targetValue: formData.targetType === 'all'
          ? 'All Students'
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="p-3 bg-gray-50 rounded-md">
              ...
              <span
                className={`flex items-center font-medium ${socketConnected ? "text-green-600" : "text-red-600"
                  }`}
              >
                {socketConnected ? (
                  <>
